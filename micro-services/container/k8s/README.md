

## MQ

[kafka-operator](https://github.com/banzaicloud/kafka-operator)

## 安装
[sealos](https://github.com/fanux/sealos?utm_source=gold_browser_extension)

## 整合方案

[rainbond](https://github.com/goodrain/rainbond)

## Package Manager
[helm3](https://github.com/helm/helm)

[Lightweight Kubernetes manager:fist](https://github.com/fanux/fist)

## service mesh

[lstio](https://github.com/banzaicloud/istio-operator)


## 扩展
[extendeddaemonset](https://github.com/DataDog/extendeddaemonset)
[基于水位线算法的 Pod AutoScaler，相比 HPA，它提供更加丰富的算法、同时能控制 Scale 的速率和策略](https://github.com/DataDog/watermarkpodautoscaler)

## 监控

[NexClipper](https://github.com/NexClipper/NexClipper)

## 日志

[logging-operator](https://github.com/banzaicloud/logging-operator)


## 安全

[cert-manager](https://github.com/jetstack/cert-manager)
[A best practices checker for Kubernetes clusters. ](https://github.com/digitalocean/clusterlint)
[Bank-Valuts 基于K8s的秘钥存储](https://github.com/banzaicloud/bank-vaults)



## 容器调试

[tracee](https://github.com/aquasecurity/tracee)



## 云原生应用

1. [quarkus Java](https://quarkus.io/)
2. [什么叫云原生](http://dockone.io/article/8881)
3. [什么是云原生应用程序](http://dockone.io/article/591)

## 学习资源

[K8s学习资源](https://github.com/kelseyhightower/kubernetes-the-hard-way)
[kuboard-press](https://github.com/fanux/kuboard-press)


## CNI

1. [Aliyun Terway](https://github.com/AliyunContainerService/terway)

## 故障流程解决

![故障解决图](images/k8s_deployment_problem_sloved_procedure.png)